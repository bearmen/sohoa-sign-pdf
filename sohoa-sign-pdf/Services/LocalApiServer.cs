...
public sealed class LocalApiServer : IAsyncDisposable
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        app.Use(async (context, next) =>
        {
            if (!IPAddress.IsLoopback(context.Connection.RemoteIpAddress ?? IPAddress.None))
            {
                ...
                await context.Response.WriteAsJsonAsync(new { error = "Ch? cho phép g?i t? localhost." }, cancellationToken);
                return;
            }
            ...
            {
                context.Response.StatusCode = StatusCodes.Status403Forbidden;
                await context.Response.WriteAsJsonAsync(new { error = "Origin không h?p l?." }, cancellationToken);
                return;
            }
        ...
        app.MapGet("/get-public-key", async (string? certId, CancellationToken ct) =>
        {
            ...
            return publicKey is null
                ? Results.NotFound(new { error = "Không tìm th?y public key." })
        ...
        _app = app;
        _logger.Info($"Local API ?ã ch?y t?i http://127.0.0.1:{config.ApiPort}");
    }
    ...
    public async Task StopAsync(CancellationToken cancellationToken = default)
    {
        ...
        _app = null;
        _logger.Warning("Local API ?ã d?ng.");
    }
