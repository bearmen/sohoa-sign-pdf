...
public sealed class TokenService : IDisposable
{
    ...
    public async Task<bool> LoginAsync(string pin, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidOperationException("PIN không ???c ?? tr?ng.");
        }
    ...
    public async Task<VerifyResult> VerifyAsync(string data, string signatureBase64, string? certId, bool inputIsBase64, string hashAlgorithm, CancellationToken cancellationToken = default)
    {
        ...
        if (selected is null)
        {
            return new VerifyResult { Error = "Không tìm th?y ch?ng th? ?? verify." };
        }
        ...
        if (rsa is null)
        {
            return new VerifyResult { Error = "Certificate không h? tr? RSA public key." };
        }
    ...
    private async Task<SignResult> SignHashInternalAsync(byte[] hash, string? certId, string? pin, string hashAlgorithm, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (selected is null)
            {
                return new SignResult { Error = "Không tìm th?y certificate phù h?p." };
            }

            var keyHandle = FindPrivateKeyHandle(selected.Id);
            if (keyHandle is null)
            {
                return new SignResult { Error = "Không tìm th?y private key trên token." };
            }
            ...
            _lastLoginUtc = DateTime.UtcNow;
            _logger.Info($"Ký d? li?u thành công v?i certificate {selected.Subject}.");
        ...
        {
            _logger.Error("Ký d? li?u th?t b?i.", ex);
    ...
    private TokenStatusInfo GetStatusInternal()
    {
        ...
        if (string.IsNullOrWhiteSpace(config.TokenLibraryPath) || !File.Exists(config.TokenLibraryPath))
        {
            return new TokenStatusInfo
            {
                ...
                Message = "Ch?a c?u hình ???ng d?n PKCS#11 DLL."
            };
        }
        ...
        try
        {
            ...
            if (slot is null)
            {
                ...
                return new TokenStatusInfo
                {
                    ...
                    Message = "Ch?a phát hi?n USB Token."
                };
            }
            ...
            return new TokenStatusInfo
            {
                ...
                Message = _isLoggedIn ? "Token s?n sàng." : "Token ?ã c?m, c?n login."
            };
        }
        catch (Exception ex)
        {
    ...
    private void EnsureLibraryLoaded()
    {
        ...
        {
            throw new FileNotFoundException("Không tìm th?y PKCS#11 library.", libraryPath);
        }
    ...
    private void EnsureActiveSlot()
    {
        ...
        {
            throw new InvalidOperationException("Không tìm th?y USB Token nào ?ang c?m.");
        }
    }
    ...
    private void EnforceSessionTimeout()
    {
        ...
        {
            _session?.Logout();
            _logger.Info("Session token ?ã t? logout do timeout.");
        }
        catch (Pkcs11Exception)
        {
        }
